...
public class AddProducts extends HttpServlet {
	...
	final String UPLOAD_DIRECTORY = "D:/Workspace/OnlineBakery/WebContent/products/";
	static int i = 0;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if (ServletFileUpload.isMultipartContent(request)) 
		{
			try 
			{
				...
				String category	="",p_name="",specification="",base_price="",mfg_date="",exp_date="";
				...
				for (FileItem item : multiparts)
				{
					if ((item.getFieldName()).equals("mfg_date")) 
					{
						mfg_date = (String) item.getString();
					}
				}
				
				for (FileItem item : multiparts)
				{
					if ((item.getFieldName()).equals("exp_date")) 
					{
						exp_date = (String) item.getString();
					}
				}
